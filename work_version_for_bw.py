...
l1,g1 = laplacian_pyramid(img1, sigma, layers)
l2,g2 = laplacian_pyramid(img2, sigma, layers)
m = gauss_pyramid(mask, sigma, layers)
...
#    l1[i] = np.clip(l1[i], 0.0, 255.0)

#for i in range(len(l2)):
#    l2[i] = np.clip(l2[i], 0.0, 255.0)

temp = []
for i in range(layers + 1):
    t = None
    t = (l1[i] * m[i]) + l2[i] * (255 - m[i])
    # t = np.clip(t, 0.0, 1.0)
    temp.append(t)
